<?php
  ...
  $pass = hash("sha256", $salt . $pass . $loggedinPerson['name'] );
        ...
        if ($loggedinPerson['password'] == $pass) {
          ...
          $warning='<div class="alert alert-warning"><strong>Fel inloggning, prova igen</strong></div>';
      }
      }
      else{
        $warning='<div class="alert alert-warning"><strong>Fel inloggning, prova igen</strong></div>';
                ...
                ?>
                 <div class="account-wall">
                  ...
                  <div class="form-group">
                     <input type="text" class="form-control" name="username" placeholder="Namn" required>
                  </div>
                  <div class="form-group">
                    <input type="password" class="form-control" name="password" placeholder="Lösenord" required>
                  </div>
                   <button class="btn btn-lg btn-primary btn-block" name="loginBtn" type="submit">Logga in</button>
                   </form>
               </div>
               <a href="register.php" class="text-center new-account">Skapa nytt konto</a>
           </div>
       </div>
   </div>
